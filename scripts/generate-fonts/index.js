...
function red(x) {
  return `\x1b[38;2;255;127;127m${x}\x1b[0m`;
}
function green(x) {
  return `\x1b[38;2;127;255;127m${x}\x1b[0m`;
}
function gray(x) {
  return `\x1b[38;2;127;127;127m${x}\x1b[0m`;
}
    ...
    output_streams[variation]
      ...
      .on("finish", async function () {
        ...
        if (ttfData) {
          ...
          await writeCss(
            path(output_folder, variation, "webfont.css"),
            family,
            variation,
            formats
          );
...
async function writeGlyph(glyphFolder, variation, meta) {
  let fileName = `${glyphFolder}_${variation}.svg`
    .toLowerCase()
    .replace(/\s/g, "_");
  ...
  try {
    ...
    fileName = `${glyphFolder}_${variation}_default.svg`
      .toLowerCase()
      .replace(/\s/g, "_");
    glyphFile = path(
      assets_folder,
